...
class RoomsTableSeeder extends Seeder
{
    ...
    function Add_Room($aF_Center_id, $aF_prop_tech_char, $aF_room_type, $aF_number, $aF_xp_code, $aF_photo, $aF_name, $aF_Description, $aF_status)
        {
        $zmEQ = new Room();
        ...
        $zmEQ->simmed_technician_character_propose_id=App\TechnicianCharacter::where('character_short',$aF_prop_tech_char)->get()->first()->id;
    ...
    function Add_Storage($aF_room, $aF_type, $aF_number, $aF_name, $aF_Description, $aF_shelf,$aF_sort)
        ...
        $ir=Add_Room($Center_id, 'free', 3, '_MW', '', '', 'Magazyn Wejściowy', 'Magazyn sprzętów przychodzących',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);

        $ir=Add_Room($Center_id, 'free', 1, 'A 1.05', 'CM_A1.05', 'a105/a1.05a.jpg', 'Umiejętności techniczne - położnictwo', 'Sala umiejętności technicznych - położnictwo',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
        ...
        $ir=Add_Room($Center_id, 'free', 1, 'A 1.06',  'CM_A1.06','a106/a1.06a.jpg', 'Umiejętności pielęgniarskie', 'Sala umiejętności pielęgniarskich',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
            for ($i=1; $i<=4; $i++)
        ...
        $ir=Add_Room($Center_id, 'free', 1, 'A 1.07', 'CM_A1.07', 'a107/a1.07c.jpg', 'Umiejętności techniczne - pielęgniarstwo', 'Sala umiejętności technicznych - pielęgniarstwo',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
            for ($i=1; $i<=3; $i++)
                Add_Storage($ir, $storage_magazynowy, 'szafka W'.str_pad($i, 2, 0, STR_PAD_LEFT), 'Szafka wisząca '.$i.'-M', 'Szafka wisząca magazynowa '.$i, 2, $sort_count++);
        ...
        $ir=Add_Room($Center_id, 'free', 4, 'A 1.08', '', 'a108/a1.08wejscie.jpg', 'Komunikacja sal', 'Komunikacja sal - pielęgniarstwo i położnictwo',$room_noactive);
        ...
        $ir=Add_Room($Center_id, 'free', 1, 'A 1.09', 'CM_A1.09', 'a109/a1.09a.jpg', 'Sala umiejętności położniczych', 'Sala umiejętności położniczych',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
        ...
        $ir=Add_Room($Center_id, 'stay', 11, 'B 1.01', 'CM_B1.01', 'b101/b1.01a.jpg', 'Wysoka Wierność Pielęgniarstwo', 'Sala Wysokiej Wierności Opieki Pielęgniarskiej',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
            for ($i=1; $i<=5; $i++)
        ...
        $ir=Add_Room($Center_id, 'free', 2, 'B 1.02', '', 'b102/b1.02.jpg', 'Pomieszczenie kontrolne (B 1.01)', 'pomieszczenie kontrolne przy sali B 1.01',$room_noactive);
            Add_Storage($ir, $storage_magazynowy, 'sala', 'sala', 'sala', 1, 0);

        $ir=Add_Room($Center_id, 'free', 2, 'B 1.03', '', 'b103/b1.03.jpg', 'Pomieszczenie kontrolne (B 1.05)', 'pomieszczenie kontrolne przy sali B 1.05',$room_noactive);
            Add_Storage($ir, $storage_magazynowy, 'sala', 'sala', 'sala', 1, 0);

        $ir=Add_Room($Center_id, 'stay', 11, 'B 1.05', 'CM_B1.05', 'b105/b1.05a.jpg', 'Wysoka Wierność Położnictwo', 'Sala Wysokiej Wierności Położnicza',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
            $sort_count=1;
            for ($i=1; $i<=6; $i++) 
                Add_Storage($ir, $storage_cwiczeniowy, 'szafka W'.str_pad($i, 2, 0, STR_PAD_LEFT), 'Szafka wisząca '.$i.'-C', 'Szafka wisząca ćwiczeniowa '.$i, 2, $sort_count++);
        ...
        $ir=Add_Room($Center_id, 'free', 3, 'B 1.06', '', 'b106/b1.06.jpg', 'mag-L', 'Magazyn (pomieszczenie lewe)',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
        $ir=Add_Room($Center_id, 'free', 3, 'B 1.07', '', 'b107/b1.07.jpg', 'mag-S', 'Magazyn (pomieszczenie środkowe)',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
        $ir=Add_Room($Center_id, 'free', 3, 'B 1.08', '', 'b108/b1.08.jpg', 'mag-K', 'Magazyn-komunikacja (pomieszczenie przejściowe)',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'korytarz', 'korytarz', 'korytarz', 1, 0);
        $ir=Add_Room($Center_id, 'free', 3, 'B 1.09', '', 'b109/b1.09.jpg', 'mag-P', 'Magazyn (pomieszczenie prawe)',$room_noactive);
            Add_Storage($ir, $storage_cwiczeniowy, 'sala', 'sala', 'sala', 1, 0);
        ...
        $ir=Add_Room($Center_id, 'free', 1, 'B 1.25', 'CM_B1.25', 'b125/b1.25a.jpg', 'Sala B 1.25', 'Sala B 1.25',$room_noactive);
